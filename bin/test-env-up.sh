...
if [ "${REDIS_PORT}" = "0" ] || [ -z "${REDIS_PORT}" ]; then
  if command -v docker &>/dev/null; then
    PORT_FROM_DOCKER=$(docker inspect -f '{{ (index (index .NetworkSettings.Ports "6379/tcp") 0).HostPort }}' fetcher-functional-redis 2>/dev/null || true)
...
# Data pipeline defaults for mocks/us_fl
  ...
  export OC_DATA_PIPELINE_STORAGE_S3_URL=${OC_DATA_PIPELINE_STORAGE_S3_URL}
