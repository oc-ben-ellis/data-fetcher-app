...
class TestStrategyConsolidated:
    ...
    @pytest.fixture
    def config_loader(
        self, strategy_registry: StrategyFactoryRegistry
    ) -> DataPipelineConfig:
    ...
    def test_strategy_registry_has_expected_factories(
        self, strategy_registry: StrategyFactoryRegistry
    ) -> None:
    ...
    def test_strategy_registry_has_expected_strategy_ids(
        self, strategy_registry: StrategyFactoryRegistry
    ) -> None:
        ...
        found_expected = [
            sid for sid in expected_strategy_ids if sid in found_strategy_ids
        ]
        assert len(found_expected) > 0, (
            f"Expected to find some of {expected_strategy_ids}, but found: {found_strategy_ids}"
        )

    def test_strategy_registry_validation_works(
        self, strategy_registry: StrategyFactoryRegistry
    ) -> None:
        ...
        with pytest.raises(Exception):
            strategy_registry.validate(
                strategy_type, strategy_id, {"invalid_param": "value"}
            )

    def test_strategy_registry_error_handling(
        self, strategy_registry: StrategyFactoryRegistry
    ) -> None:
    ...
    def test_create_strategy_from_config_signature(
        self, config_loader: DataPipelineConfig
    ) -> None:
        """Test that _create_strategy_from_config has the correct signature."""
        method = config_loader._create_strategy_from_config
    ...
    def test_create_strategy_from_config_with_valid_data(
        self, config_loader: DataPipelineConfig
    ) -> None:
        ...
        strategy_type = registered_types[0]
        strategy_ids = list(
            config_loader.strategy_registry._factories[strategy_type].keys()
        )
        assert len(strategy_ids) > 0
        ...
        try:
            result = config_loader._create_strategy_from_config(
                strategy_type=strategy_type, strategy_id=strategy_id, strategy_config={}
            )
            ...
            assert any(
                ...
                for keyword in ["required", "missing", "invalid", "validation"]
            )

    def test_create_strategy_from_config_error_handling(
        self, config_loader: DataPipelineConfig
    ) -> None:
        ...
        with pytest.raises(Exception):
            config_loader._create_strategy_from_config(
                strategy_type=str, strategy_id="test_id", strategy_config={}
            )
        ...
        with pytest.raises(Exception):
            config_loader._create_strategy_from_config(
                strategy_type=None, strategy_id="test", strategy_config={}
            )
    ...
    def test_strategy_registry_contains_expected_types(
        self, strategy_registry: StrategyFactoryRegistry
    ) -> None:
        ...
        print(
            f"✓ Found {len(http_strategies)} HTTP strategies and {len(sftp_strategies)} SFTP strategies"
        )
    ...
    def test_protocol_config_basic_validation(
        self, strategy_registry: StrategyFactoryRegistry
    ) -> None:
...
"""
    ...
    def test_strategy_registry_validation_basic(
        self, strategy_registry: StrategyFactoryRegistry
    ) -> None:
