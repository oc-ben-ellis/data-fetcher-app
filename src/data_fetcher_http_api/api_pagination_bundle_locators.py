"""Pagination-aware bundle locator implementations.

This module provides bundle locators that handle paginated data sources,
including support for various pagination strategies and cursor-based navigation.
"""
...
class PaginationStrategy:
    ...
    def should_continue_pagination(self, response_data: dict[str, Any]) -> bool:
        """Determine if pagination should continue based on response data."""
        raise NotImplementedError

    def get_next_cursor(self, response_data: dict[str, Any]) -> str | None:
        """Get the next cursor from response data."""
        raise NotImplementedError

    def should_narrow_search(self, response_data: dict[str, Any]) -> bool:
        """Determine if search should be narrowed."""
        raise NotImplementedError
...
class CursorPaginationStrategy(PaginationStrategy):
    ...
    cursor_field: str = "curseurSuivant"
    total_field: str = "total"
    count_field: str = "nombre"
    max_records: int = 20000

    def should_continue_pagination(self, response_data: dict[str, Any]) -> bool:
        """Check if there are more pages to fetch."""
        header = response_data.get("header", {})
        total = header.get(self.total_field, 0)
        count = header.get(self.count_field, 0)
        next_cursor = header.get(self.cursor_field)

        return bool(total <= self.max_records and next_cursor and count > 0)

    def get_next_cursor(self, response_data: dict[str, Any]) -> str | None:
        """Get the next cursor from response."""
        header = response_data.get("header", {})
        cursor = header.get(self.cursor_field)
        if isinstance(cursor, str):
            return cursor
        return None

    def should_narrow_search(self, response_data: dict[str, Any]) -> bool:
        """Check if search should be narrowed."""
        header = response_data.get("header", {})
        total = header.get(self.total_field, 0)
        return bool(total > self.max_records)


@dataclass
class ComplexPaginationHttpBundleLocator:
    """Generic bundle locator for APIs with complex pagination logic."""
    ...
    state_management_prefix: str = "complex_pagination_provider"

    def __post_init__(self) -> None:
        """Initialize the complex pagination bundle locator state and internal variables."""
        self._processed_urls: set[str] = set()
        self._url_queue: list[str] = []
        self._current_date: date | None = None
        self._current_cursor: str = "*"
        self._current_narrowing: str | None = None
    ...
    def set_app_config(self, app_config: AppConfig) -> None:
        """Set the app config for this locator."""
        self._app_config = app_config

        # Set default pagination strategy if none provided
        if self.pagination_strategy is None:
            self.pagination_strategy = CursorPaginationStrategy()
    ...
    async def _load_persistence_state(self, context: FetchRunContext) -> None:
        ...
        if state_data and isinstance(state_data, dict):
            ...
            self._current_narrowing = state_data.get("current_narrowing")
    ...
    async def _generate_urls_for_current_date(self) -> None:
        ...
        if self.query_builder:
            params["q"] = self.query_builder(date_str, self._current_narrowing)
        else:
    ...
    async def _generate_next_urls(self, _ctx: FetchRunContext) -> None:
        ...
        if (
            ...
            and self.date_end
            and (
                self._current_date < datetime.strptime(self.date_end, "%Y-%m-%d").date()  # noqa: DTZ007
            )
        ):
            self._current_date += timedelta(days=1)
            self._current_cursor = "*"  # Reset cursor for new date
...
@dataclass
class ReversePaginationHttpBundleLocator:
    """Bundle locator for reverse pagination (from end to start)."""
    ...
    pagination_strategy: PaginationStrategy | None = None
    narrowing_strategy: Callable[[str], str] | None = None
    state_management_prefix: str = "reverse_pagination_provider"

    def __post_init__(self) -> None:
        """Initialize the reverse pagination bundle locator state and internal variables."""
        self._processed_urls: set[str] = set()
        self._url_queue: list[str] = []
        self._current_date: date | None = None
        self._current_cursor: str = "*"
    ...
    async def _load_persistence_state(self, context: FetchRunContext) -> None:
        ...
        if state_data and isinstance(state_data, dict):
            self._current_date = datetime.strptime(  # noqa: DTZ007
                state_data.get(
                    "current_date",
                    self.date_end or datetime.now(tz=UTC).date().strftime("%Y-%m-%d"),
                ),
                "%Y-%m-%d",
            ).date()
            self._current_cursor = state_data.get("current_cursor", "*")
    ...
    async def _save_persistence_state(self, context: FetchRunContext) -> None:
        ...
        state_data = {
            "current_date": (
                self._current_date.strftime("%Y-%m-%d")
                if self._current_date
                else (self.date_end or datetime.now(tz=UTC).date().strftime("%Y-%m-%d"))
            ),
            "current_cursor": self._current_cursor,
            "current_narrowing": self._current_narrowing,
            "initialized": self._initialized,
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            self._initialized = True
            logger.info(
                "Initialized reverse pagination provider",
                date_start=start_date,
                date_end=end_date,
                base_url=self.base_url,
            )
